...
func (r *Router) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	// 1. NEU: Check auf Static Files (VOR dem dynamischen Routing)
	// Wir prüfen: Fängt der Pfad mit "/assets" (oder ähnlichem) an?
	for prefix, handler := range r.staticHandlers {
		if strings.HasPrefix(req.URL.Path, prefix) {
			// Wenn ja: Lass den FileServer die Datei ausliefern und beende hier.
			handler.ServeHTTP(w, req)
			return
		}
	}

	// 2. Trie Lookup (Dynamische Routen suchen)
	...
	// 4. NEU: Templates in den Context injizieren!
	if r.templates != nil {
...
func (r *Router) LoadHTMLGlob(pattern string) {
	if r.templates == nil {
		// Erstelle ein leeres Template und registriere die FuncMap
